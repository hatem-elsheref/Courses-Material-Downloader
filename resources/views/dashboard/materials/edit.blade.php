...
@section('bread')
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{route('course.show',$material->course_id)}}"><i class="fa fa-graduation-cap"></i> Courses Materials</a></li>
        ...
        <div class="row">
            ...
            <!-- general form elements -->
                <div class="box box-primary">
                    ...
                    <!-- form start -->
                    <form role="form" method="post" action="{{route('material.update',$material->id)}}" enctype="multipart/form-data">
                        @csrf
                        @method('put')
                        ...
                        <div class="box-body">
                            <div class="form-group">
                                ...
                                <input  type="text" class="form-control @error('title') is-invalid @enderror" name="title" placeholder=" Enter The Material title" value="{{$material->title}}" >
                                @error('title')
                            ...
                            <div class="form-group">
                                ...
                                <select name="year" class="form-control custom-select" required>
                                    ...
                                    @foreach(range(1995,date('Y')) as $year)
                                        <option value="{{$year}}" {{($material->year == $year)?'selected':''}}>{{$year}}</option>
                                    @endforeach
                                </select>
                                @error('year')
                                <div class="invalid-feedback text-danger">{{$message}}</div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="part" class="form-control custom-select" required>
                                    ...
                                    @foreach(range(1,100) as $part)
                                        <option value="{{$part}}" {{($material->part == $part)?'selected':''}}>{{$part}}</option>
                                    @endforeach
                                </select>
                                @error('part')
                                <div class="invalid-feedback text-danger">{{$message}}</div>
                            ...
                            <div class="form-group">
                                ...
                                <select name="source" class="form-control custom-select" id="source" required>
                                    ...
                                    @foreach($sources as $source)
                                        <option value="{{$source}}" {{($material->source == $source)?'selected':''}}>{{$source}}</option>
                                    @endforeach
                                </select>
                                @error('source')
                                <div class="invalid-feedback text-danger">{{$message}}</div>
                            ...
                            <div class="col-sm-5">
                                ...
                                <input  type="text" class="form-control @error('material_external_host') is-invalid @enderror" name="material_external_host" placeholder=" Enter The Link Name" value="@if($material->type == 'External_Host') {{ $material->path }}   @endif" >
                                @error('material_external_host')
                                <div class="invalid-feedback text-danger">{{$message}}</div>
                                @enderror
                            </div>
                            <div class="col-sm-5" id="for-youtube">
                                <label for="material_youtube">Video Id (Youtube)</label>
                                <input  type="text" class="form-control @error('material_youtube') is-invalid @enderror" name="material_youtube" placeholder=" Enter The Youtube Video Id" value="@if($material->type == 'Youtube') {{ $material->path }}   @endif" >
